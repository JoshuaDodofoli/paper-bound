...
import { ArrowUpRight, Book, Search, User } from "lucide-react";
import Link from "next/link";
import { useState } from "react";
import Modal from "@/components/overlays/Modal";
import { searchCatalog } from "@/domain/catalog/queries";
...
const SearchModal = ({ isSearchOpen, setIsSearchOpen }: SearchModalProps) => {
  ...
  const filteredResults = query.length > 0 ? searchCatalog(query) : [];
  ...
  return (
    ...
    >
      <div className="-mt-2">
        <span className="text-xs text-dark-grey/40 font-bold uppercase tracking-widest mb-2 block">
          search through our library
        </span>
          ...
          <AnimatePresence mode="wait">
            {query.length === 0 ? (
              <motion.div
                ...
                className="space-y-4"
              >
                <h4 className="text-xs text-dark-grey/40 font-bold uppercase tracking-widest ml-1">
                  Quick Suggestions
                </h4>
              ...
              >
                <div className="flex items-center justify-between ml-1 mb-2">
                  <h4 className="text-xs text-dark-grey/40 font-bold uppercase tracking-widest">
                    Results for &quot;{query}&quot;
                  </h4>
